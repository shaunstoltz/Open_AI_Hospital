...
class CollaborativeConsultation:
    ...
    def add_parser_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument("--translated", default=True, type=bool, help="medical json already translated to english")
    ...
    def _run(self, patient):
        ...
        for i, doctor in enumerate(self.doctors):
            revise_diagnosis_by_symptom_and_examination = doctor.revise_diagnosis_by_symptom_and_examination(
            ...
            diagnosis_in_turn.append({
                ...
                "revise_diagnosis_by_symptom_and_examination": revise_diagnosis_by_symptom_and_examination
            })
            if self.translate:
                print(i, doctor.name, doctor.get_diagnosis_by_patient_id(patient.id, "Diagnosis", translate=self.translate))

            else:
                print(i, doctor.name, doctor.get_diagnosis_by_patient_id(patient.id, "诊断结果"))
        if self.ff_print:
            print("-"*100)
            ...
            for k in range(self.max_discussion_turn):
                ...
                for i, doctor in enumerate(self.doctors):
                    ...
                    diagnosis_in_turn.append({
                        "doctor_id": i,
                        "doctor_engine_name": doctor.engine.model_name,
                        "diagnosis": doctor.get_diagnosis_by_patient_id(patient.id, translate=self.translate),
                        "revise_diagnosis_by_others": revise_diagnosis_by_others
                    })
                    if self.ff_print:
                        if self.translate:
                            print(k, i, doctor.name, doctor.get_diagnosis_by_patient_id(patient.id, "Diagnosis", translate=self.translate))
        ...
        diagnosis_info = {
            ...
            "diagnosis_in_turn": diagnosis_in_discussion
        }
