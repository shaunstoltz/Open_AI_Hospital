    ...
    def load_doctor_name_to_scores(self, evaluation_result_path, load_diagnosis=False):
        ...
        with jsonlines.open(evaluation_result_path, "r") as reader:
            for obj in reader:
                ...
                sympton_choice, test_choice = obj.get("symptom_choice"), obj.get("test_choice")
                sympton_score, test_score = char_to_score.get(sympton_choice), char_to_score.get(test_choice)
                sympton_score = sympton_score if sympton_score is not None else 0
                test_score = test_score if test_score is not None else 0
                ...
                diagnosis_score = diagnosis_score if diagnosis_score is not None else 0
                basis_score = basis_score if basis_score is not None else 0
                treatment_score = treatment_score if treatment_score is not None else 0
                if doctor_name not in doctor_name_to_scores:
                    ...
                    if sum([diagnosis_score, basis_score, treatment_score, sympton_score, test_score]) > 3:
    ...
    def show_result(self):
        ...
        for doctor_name, scores in doctor_name_to_scores.items():
            ...
            for i in [4, 5, 1, 2, 3]:
                ...
                print("{}: mean: {:.3f}, ci: ({:.3f}, {:.3f}), range: {:.3f}".format(
                    idx_to_part.get(i-1), results.value, results.lower_bound, results.upper_bound, b))
...
def main():
    ...
    parser.add_argument("--onestep_evaluation_result_path", type=str, default="../outputs/evaluation/archive/evaluation_iiyi_gpt4_onestep.jsonl")
